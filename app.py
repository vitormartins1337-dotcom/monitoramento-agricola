...
st.set_page_config(page_title="Agro-Intel Visual", page_icon="🧬", layout="wide", initial_sidebar_state="expanded")

# --- FUNÇÃO PARA O FUNDO (BACKGROUND) ---
def get_base64_of_bin_file(bin_file):
    """Lê a imagem local e converte para string base64"""
    with open(bin_file, 'rb') as f:
        data = f.read()
    return base64.b64encode(data).decode()

def set_background(png_file):
    """Aplica a imagem como fundo com uma camada branca semitransparente para leitura"""
    try:
        # Tenta carregar a imagem local se ela existir
        bin_str = get_base64_of_bin_file(png_file)
        imagem_url = f"data:image/png;base64,{bin_str}"
    except:
        # Fallback: Se não achar a imagem local, usa uma URL online parecida (Drone/Agro)
        imagem_url = "https://images.unsplash.com/photo-1625246333195-78d9c38ad449?q=80&w=1740&auto=format&fit=crop"

    page_bg_img = f'''
    <style>
    .stApp {{
        background-image: linear-gradient(rgba(255,255,255,0.88), rgba(255,255,255,0.88)), url("{imagem_url}");
        background-size: cover;
        background-position: center;
        background-attachment: fixed;
    }}
    
    /* Melhoria nos Cards para destacar do fundo */
    ...
    .radar-card {{ background-color: rgba(225, 245, 254, 0.9); padding: 10px; border-radius: 5px; text-align: center; border: 1px solid #b3e5fc; }}
    .alert-high {{ background-color: rgba(255, 235, 238, 0.95); border-left: 5px solid #c62828; padding: 15px; border-radius: 5px; color: #b71c1c; }}
    .alert-low {{ background-color: rgba(232, 245, 233, 0.95); border-left: 5px solid #2e7d32; padding: 15px; border-radius: 5px; color: #1b5e20; }}
    
    /* Estilo das Abas */
    .stTabs [data-baseweb="tab-list"] {{ background-color: rgba(255,255,255,0.6); padding: 5px; border-radius: 10px; }}
    .stTabs [aria-selected="true"] {{ background-color: #01579b; color: white; }}
    </style>
    '''
    st.markdown(page_bg_img, unsafe_allow_html=True)

# APLICA O FUNDO (Certifique-se que o nome do arquivo aqui é igual ao que você salvou)
set_background('fundo_agro.jpg')

# --- 2. ENCICLOPÉDIA AGRONÔMICA (MANTIDA) ---
BANCO_MASTER = {
...
# --- 5. DASHBOARD ---
st.title("🛰️ Agro-Intel Visual v19.0")
...
if val_w:
    ...
    if not df.empty:
        ...
        with tabs[0]:
            ...
            # GDA BOX
            st.markdown(f"""
            <div style="background-color:rgba(255, 243, 224, 0.95); padding:15px; border-radius:8px; text-align:center; border:1px solid #ffe0b2;">
            ...
            if progresso_maturacao >= 1.0:
                st.success("✅ Maturação atingida! Verifique ponto de colheita.")
        ...
        with tabs[2]:
            ...
            if not df_radar.empty:
                ...
                for idx, row in df_radar.iterrows():
                    ...
                    # Ajuste de opacidade no card do radar
                    with cols[idx]:
                        st.markdown(f"""<div class="radar-card" style="background-color: {cor}e6"><b>{row['Direcao']}</b><br><span style="font-size: 1.5em">{row['Temp']:.0f}°C</span><br>{row['Clima']}<br><small>Chuva: {row['Chuva']}</small></div>""", unsafe_allow_html=True)
